...
vignette: >
  %\VignetteIndexEntry{Using CVD Prevent}
...
The CVD Prevent audit publishes data approximately four times per year. To view the four most recent reporting periods for **standard indicators**, use the `cvd_time_period_list()` function:

```{r}
#| label: listing time periods 1
...
There are two main types of indicator reported:
-   "Standard" (IndicatorTypeID = 1)

-   "Outcomes" (IndicatorTypeID = 2) 

To retrieve time periods specific to an indicator type, pass the relevant ID to the optional `indicator_type_id` parameter:

```{r}
#| label: listing time periods 2
...
Audit data is reported across multiple geographic levels (referred to as **System Levels**), ranging from individual GP practices to national-level aggregates (e.g., England). To view which system levels are available for each time period, use the `cvd_time_period_system_levels()` function:

```{r}
#| label: listing system levels per period 1
...
```{r}
#| label: list available system levels for a given time period
...
```{r}
#| label: list all available reporting periods for each system level
...
```{r}
#| label: list areas for a time period and system level or parent area
...
```{r}
#| label: view details for a specific area 1
...
```{r}
#| label: view details for a specific area 2
...
```{r}
#| label: view details for a specific area 3
...
```{r}
#| label: view details for a specific area 4
...
```{r}
#| label: list areas without parent details 1
...
```{r}
#| label: list areas without parent details 2
...
```{r}
#| label: search for area by keyword
...
To retrieve details for a specific area, including all nested system areas within it, you can use the function `cvd_area_nested_subsystems()`. This function requires an `area_id` as input. 

In the example below, we request details for Greater Manchester ICB (`area_id = 8030`), which returns a named list of tibbles representing heirarchical levels:

```{r}
#| label: area details - nested 1
return_list <- cvd_area_nested_subsystems(area_id = 8030)
return_list |> summary()
```

The tibble `level_1` provides details about our requested area itself. you can view it using:

```{r}
#| label: area details - nested 2
return_list$level_1 |>
  gt::gt()
```

The next level, `level_2`, contains areas that are direct children of the requested area. Here, we display hte first four entries:

```{r}
#| label: area details - nested 3
return_list$level_2 |>
  dplyr::slice_head(n = 4) |>
  gt::gt()
```

Subsequent levels (`level_3`, `level_4`, etc.) represent further nested areas - children of children - and can be explored in the same way.
...
```{r}
#| label: listing metrics for each indicator
...
```{r}
#| label: list all indicator data for a given area
...
```{r}
#| label: meta-data for an indicator
...
To access all metric data for a given area, time period and indicator you can use the `cvd_indicator_data()` function. The return from the API includes three sets of data which are grouped together as three tibbles within a named list object. The three tibbles include:
...
```{r}
#| label: all indicator data for a given time and area
...
We can extract the tibbles using the list\$object notation. Here we can see definitions for the indicator and its associated metrics.

```{r}
#| label: all indicator metrics
returned_list$indicator_metrics |> dplyr::glimpse()
```

### Area data

Here we can obtain rates of treatment with anticoagulants broken down by sex for Leicester Central PCN:

```{r}
#| label: indicator area data
...
These splits by local and national level allow for fine-grained benchmarking.
...
```{r}
#| label: all indicator metric data for system level and time period
...
```{r}
#| label: benchmark metric data with national results
...
```{r}
#| label: list priority group indicators
...
```{r}
#| label: list pathway group indicators
...
```{r}
#| label: list indicator group indicators
...
```{r}
#| label: return time series data for a metric and area 1
...
```{r}
#| label: return time series data for a metric and area 2
cvd_indicator_metric_timeseries(metric_id = 130, area_id = 705) |>
...
```{r}
#| label: return time series data for an indicator and area - inequalities 1
...
```{r}
#| label: return time series data for an indicator and area - inequalities 2
cvd_indicator_person_timeseries(indicator_id = 7, area_id = 705) |>
  dplyr::filter(
    MetricCategoryTypeName == 'Age group',
    !is.na(Value)
...
```{r}
#| label: return system level comparisons for a metric
